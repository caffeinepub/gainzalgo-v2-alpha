...
async function fetchGoldPrice(): Promise<number | null> {
  // Source 1: gold-api.com — dedicated real-time spot gold
  try {
  ...
  // Source 3: PAX Gold on CoinGecko (tokenized gold, tracks spot)
  try {
    ...
    const data = await res.json();
    if (data["pax-gold"]?.usd) return Number(data["pax-gold"].usd);
  } catch {
    // fall through
  }
...
async function fetchForexRates(): Promise<Record<string, number> | null> {
  // Source 1: Frankfurter (ECB-backed, reliable forex)
  try {
    const res = await fetch(
