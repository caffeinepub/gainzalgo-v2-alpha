...
function TickerStrip({
  ...
  const tickerItems = SYMBOLS.map((sym) => {
    ...
    return {
      sym,
      priceStr: formatPrice(sym, p.price),
      changeStr: p.changePercent,
      ...
      >
        {doubled.map((t, i) => (
          ...
          >
            ...
            <FlashPrice
              value={t.stale ? `~${t.priceStr}` : t.priceStr}
              direction={t.direction}
              className={`text-xs ${t.stale ? "text-muted-foreground" : "text-muted-foreground"}`}
            />
      ...
      <div className="px-5 py-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
        {priceGridItems.map((item) => (
          ...
          >
            ...
            {item.label === "Current" && pipsFromEntry && (
              <div
                className="text-xs font-mono mt-0.5"
                style={{
                  color: pipsFromEntry.startsWith("+")
                    ? "oklch(0.69 0.190 145)"
                    : "oklch(0.58 0.220 27)",
                }}
              >
  ...
  return (
    <div className="min-h-screen">
      ...
      <HeroSection
        ...
        lastUpdated={lastUpdated}
      />
      <TickerStrip prices={prices} loading={loading} />
